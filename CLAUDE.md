...
- **Auth guards read fresh from Mongo, not the JWT**: `requireActiveUser()` / `requireAdmin()` (`src/server/auth/session.ts`) re-check `status`/`platformRole`/`functionalRoleId` from the DB on every protected request, so deactivation/role changes take effect immediately. `src/proxy.ts` only does an optimistic "is there a JWT" check for UX redirects — it is not a security boundary; don't rely on it for authorization.
- **Content lifecycle** is `DRAFT → PUBLISHED → ARCHIVED` (terminal), enforced by `assertValidTransition` (see `src/lib/content-status.ts`). Visibility cascades: Route `PUBLISHED` → Stage `PUBLISHED` → Item/Process `PUBLISHED` + matching `scope`/`roleIds`. Breaking any link in that chain hides the content (`resolveVisibleContent`/`resolveVisibleProcesses`/`resolveVisibleSteps`).
...
- **`onboarding_routes` is a tenant singleton** (unique index on `{tenantId}` alone) — one route per tenant, not per functional role; role-specific content lives in `scope`/`roleIds` on items/leaders/processes, not in route structure. Archiving the route currently has no undo path (see BACKLOG.md).
...
**`BACKLOG.md`** lists features explicitly deferred by product-owner decision (quiz-type content, invitation revoke/resend, promote-user-to-admin, client-direct-upload to Blob, route un-archiving, broader rate limiting/observability). Treat these as out of scope unless the user explicitly asks for one — don't implement them opportunistically while touching nearby code.
