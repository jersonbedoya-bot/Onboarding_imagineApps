...
- **User deletion** follows the same two-step lifecycle convention as content/stage/leader/process/step (`archive → delete`, never in one step): `user.service.deleteUser` only accepts a target already `INACTIVE` (`deactivateUser` first) and is a hard Mongo delete (`DELETE /api/users/{id}`) — past `audit_logs` and `user_progress` records are never cascaded, same precedent as `deleteContentItem`.
