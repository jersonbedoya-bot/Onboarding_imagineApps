...
- **`content_items.displayFormat`** (`PROSE` default / `FACT_GRID` / `VALUES_GRID` / `TIMELINE` / `STEPS` / `QUIZ`) drives how `body` renders in `OnboardingJourney.tsx` — an explicit admin-chosen field (dropdown in `ContentForm.tsx`), not a guess by title. Each non-`PROSE` format expects a specific Markdown pattern inside `body` (documented per-parser in `src/lib/content-display.ts`); if the text doesn't match, it silently falls back to plain `MarkdownContent` — never breaks the view. `FACT_GRID`/`STEPS` render via the shared `IconCardGrid` (always-visible cards); `VALUES_GRID` is the one deliberate exception with a click-to-expand interaction (`CultureValuesGrid`, for culture/values content meant to be read reflectively, not scanned); `TIMELINE` is only for genuinely chronological content.
- **Quiz gate**: a content item with `displayFormat: "QUIZ"` is rendered as a `QuizBlock` (options shuffled client-side per mount) and gates the "next module" action in `OnboardingJourney.tsx` — all of its questions must be answered (correctness doesn't matter) before the advance button enables. Once passed, the item's `view` progress record is written (`POST /api/progress/content/{id}/view`, the same endpoint `ContentViewTracker` uses for INFORMATIONAL content) so the user isn't gated again on return visits; the individual answers/correctness are never persisted, only the fact it was answered once.
- **Invitations**: the raw token is shown once; only its SHA-256 hash is persisted (`src/lib/token.ts`). Functional role is always assigned via the invitation, never chosen by the user; an `ADMIN` invitation must have `functionalRoleId: null`. A still-`PENDING` invitation can be revoked (`invitation.service.revokeInvitation`, `POST /api/invitations/{id}/revoke`) to free its email immediately instead of waiting the 7-day TTL — no separate "resend" endpoint exists because `createInvitation`'s duplicate-email check only blocks on a `PENDING` + non-expired invitation, so revoking (or letting it expire) is enough for `+ Invitar usuario` to accept that email again. The public token routes (`GET/POST /api/invitations/{token}...`) and this admin-only `{id}` route share one folder (`src/app/api/invitations/[id]/`) because Next.js forbids two dynamic-segment names at the same path level — the folder name is just routing plumbing, unrelated to which value (raw token vs Mongo `_id`) each route actually receives.
- **User deletion** (`user.service.deleteUser`, `DELETE /api/users/{id}`) is a hard Mongo delete, allowed straight from `ACTIVE` (no forced deactivate-first step) — guarded the same way as `changePlatformRole`: can't delete yourself, and can't delete the tenant's only active `ADMIN`. Past `audit_logs` and `user_progress` records are never cascaded, same precedent as `deleteContentItem`.
...
- **Markdown-formatted fields** (`content_items.body`, `processes.objective`/`context`/`expectedResult`, `process_steps.description`/`instruction`) render via `MarkdownContent` (react-markdown + remark-gfm); their admin edit control is `MarkdownTextarea`, which always shows the raw textarea and its rendered result side by side (no more "click to preview" — the point is that breaking a `**bold**`/`[link](url)` is visible immediately, since a non-technical admin editing that same text can silently strip it). `src/lib/markdown-guard.ts` backs a second, non-blocking confirmation (via `ConfirmModal` with `tone="neutral"`) when saving an edit whose bold/link count went down versus the stored value — heuristic (occurrence count, not balanced-syntax parsing), applies only in edit mode (no "before" to compare on create).
...
- **Zod schemas** for every entity live in `src/server/validation/*` and are the API-edge validation; MongoDB's `$jsonSchema` validators in `schema.ts` are defense-in-depth behind them, not a substitute. Every route handler that does `schema.safeParse(body)` throws `new ValidationError(zodErrorMessage(parsed.error, "<generic fallback>"), parsed.error.flatten())` (`src/lib/zod-error.ts`) — `zodErrorMessage` surfaces the first real Zod issue message (e.g. "Debe contener al menos un número.") as the error's top-level `message`, since every client component only reads `body?.error?.message` and never `details.fieldErrors`. Without this, a rejected form only ever showed its generic fallback string, with no way to tell which field or rule actually failed (reported directly by the user, who resorted to guessing at a password's actual requirements).
